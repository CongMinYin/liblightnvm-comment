...
void ewr_s12_1addr(int use_meta)
{
	...
	for (size_t pg = 0; pg < geo->npages; ++pg) {
		...
		//gettimeofday( &end, NULL );
		//printf("once write block diff: %d.%d\n", end.tv_sec - start.tv_sec, end.tv_usec - start.tv_usec);
		if (res < 0) {
	...
	gettimeofday( &end, NULL );
	printf("write size:%d x %d x %lu byte diff: %d.%d\n", geo->npages, naddrs, buf_w_nbytes, end.tv_sec - start.tv_sec, end.tv_usec - start.tv_usec);
	...
	gettimeofday( &end, NULL );
	gettimeofday( &end, NULL );
	printf("write size:%d x %d x %d x %lu byte diff: %d.%d\n", geo->npages, geo->nplanes,  geo->nsectors, buf_r_nbytes, end.tv_sec - start.tv_sec, end.tv_usec - start.tv_usec);
