...
void ewr_s12_1addr(int use_meta)
{
	...
	gettimeofday( &end, NULL );
	printf("write size:%lu x %lu = %lu byte diff: %ld\n", geo->npages, buf_w_nbytes, geo->npages*buf_w_nbytes, (end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec);

		gettimeofday( &start, NULL );
		res = nvm_cmd_write(dev, addrs, naddrs, buf_w,
				     use_meta ? meta_w : NULL, pmode, &ret);
    gettimeofday( &end, NULL );
		printf("single write size:%lu diff: %ld\n", buf_w_nbytes, (end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec);
  ...
  // 读取是三层循环，plane层循环放中间也是为了并行性
	...
	gettimeofday( &end, NULL );
	printf("read size:%lu x %lu x %lu x %lu = %lu byte diff: %ld\n", geo->npages, geo->nplanes,  geo->nsectors, buf_r_nbytes, geo->npages*geo->nplanes*geo->nsectors*buf_r_nbytes,  (end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec);
		...
		res = nvm_cmd_read(dev, &addr, 1, buf_r,
						    use_meta ? meta_r : NULL, pmode, &ret);
    gettimeofday( &end, NULL );
		printf("single read size:%lu diff: %ld\n", buf_r_nbytes, (end.tv_sec - start.tv_sec) * 1000000 + end.tv_usec - start.tv_usec);
	failed = 0;
	printf("Success\n");
  return ;
out:
